...
class ETRIDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        with open(osp.join(opt.dataroot, self.datamode+'512', 'wearing_info_'+self.datamode+'_front.json'), 'r') as f:
    ...
    def __getitem__(self, index):
        ...
        Model_Image_file = osp.join(self.root,self.datamode+'512','Model-Image_resized',s_file) 
        Model_Parse_file = osp.join(self.root,self.datamode+'512', 'Model-Parse_png_resized', s_file.replace('.jpg','.png'))
        Item_Image_file =''
        Item_Parse_file =''
        if self.stage == 'GMM_ETRI' or self.stage == 'GMMBODY_ETRI':
            Item_Image_file = osp.join(self.root,self.datamode+'512','Item-Image_resized',ann['main_top']+'_F.jpg')
            Item_Parse_file = osp.join(self.root,self.datamode+'512','Item-Parse_png_resized',ann['main_top']+'_F.png')
        elif self.stage == 'TOM_SNPATCH_ETRI':
            Item_Image_file = osp.join(self.root,self.datamode+'512','Item-Image_resized_warp_'+self.opt.posfix_warp,ann['main_top']+'_F'+'__'+s_file)
            Item_Parse_file = osp.join(self.root,self.datamode+'512','Item-Parse_png_resized_warp_'+self.opt.posfix_warp,ann['main_top']+'_F'+'__'+s_file[:-3]+'png')           
           
        Keypoints_file = osp.join(self.root,self.datamode+'512','Model-Pose_resized',s_file[:-4]+'.json')
